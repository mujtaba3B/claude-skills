...
- **`/distill-question-and-answer-log-to-principles`** . Companion to a small PostToolUse hook that captures every `AskUserQuestion` answer into a pending JSONL in your auto-memory directory. This skill walks the pending log, classifies each entry (new principle, reinforces existing, contradicts existing, or one-off), surfaces proposed `question_and_answer_decision_*.md` files one at a time, and writes only what you approve. Enforces a strict conflict hierarchy: never auto-writes a principle that contradicts your `CLAUDE.md` or a `feedback_*.md`. Runs as a step inside `/close-out`; also invocable standalone. The capture hook script is not bundled in this repo (it lives in `~/.claude/scripts/`); see the skill body for how to wire it.
